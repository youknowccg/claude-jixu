...
2. **Copy to the skills directory** (only the three named files below — do **not** copy or execute anything else from this repository):
   - Target `skills/jixu/`:
     - Windows: `$USERPROFILE\.claude\skills\jixu\`
     - Unix-like: `~/.claude/skills/jixu/`
   - Copy `SKILL.md` → `skills/jixu/SKILL.md`
   - Copy `scripts/check-scripts-manifest.js` → `skills/jixu/scripts/check-scripts-manifest.js`
   - Copy `scripts/ack-writer.js` → `skills/jixu/scripts/ack-writer.js`
...
6. **Verify**:
   ...
   - `node skills/jixu/scripts/check-scripts-manifest.js` (run once against any project dir containing .py/.js/.sh/.bat files) — no errors. Note: this script only reads project files and its own manifest; it never executes project scripts.
